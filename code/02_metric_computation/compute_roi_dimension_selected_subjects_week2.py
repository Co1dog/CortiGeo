"""
This script computes ROI-level intrinsic dimension values for selected subjects
in Week 2 fMRI data. For each specified subject, ROI, and video, it loads the
...
videos, and saves separate raw-signal and difference-signal dimension CSV files
for each subject.
"""
...
VIDEOS = [1, 2, 3, 4, 5]
N_NEIGHBORS = 10
SELECTED_SUBJECTS = np.array(["s107", "s122", "s105"], dtype=str)
...
for subject_id in SELECTED_SUBJECTS:
    print(f"\n[Week {WEEK_ID}] Starting subject sub-{subject_id}")
    ...
    for region_index, region in enumerate(regions, start=1):
        print(
            f"[Week {WEEK_ID} | sub-{subject_id}] "
            f"Computing region {region_index}/{len(regions)}: {region}"
        )
    ...
    raw_csv = f"dimensions_week{WEEK_ID}_raw_sub-{subject_id}.csv"
    diff_csv = f"dimensions_week{WEEK_ID}_diff_sub-{subject_id}.csv"
